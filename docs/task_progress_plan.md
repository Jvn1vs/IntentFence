...
### 2026-09-11：ToolSafety 语义适用性探查

已完成固定词项探查及五个前缀的 AI 语境观察，确认关键词命中不能代替风险类别；未确认另外三类风险的样本覆盖。证据与下一阶段具体范围见 `reports/data/toolsafety_semantic_fitness_20260911.md`。当前停在只读审计阶段：建议获得明确确认后，生成最多 200 条隔离动作前缀复核包并进行独立标签预审；不合并训练候选、不执行训练。原五类风险和动作感知多任务目标保持不变。
