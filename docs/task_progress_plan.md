...
- ✅ 用户明确由自己执行所有真实模型训练；Codex 只搭建、验证框架并分析用户提供的日志。
...
| C1 | 用户操作当前 CPU | 真实数据、审计与规则/TF-IDF；Codex 只提供框架 | 用户自行执行 |
...
C1 工程框架完成后提交并推送，然后暂停。下一步不是由 Codex 进入训练阶段，而是项目所有者按 `docs/c1_user_runbook.md` 执行真实数据下载、转换、审计、划分和基线。用户提供 manifest、报告和日志后，Codex 只做只读复核与框架修正；C1 研究出口通过前不得进入真实训练结论阶段。
