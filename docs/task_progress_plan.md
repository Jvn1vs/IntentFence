...
- ✅ 用户已于 2026-08-20 批准冻结，协议状态为 `frozen`；C1 仍需单独确认后才开始。
...
C0 协议 1.0.0 已获用户批准并冻结，本阶段结束后暂停。下一阶段为 **C1：数据获取、审计、隔离划分与基线框架**，必须再次得到用户明确确认。C1 可下载已批准的公开数据并验证数据/评测框架，但 Codex 不拟合真实数据上的学习模型。
