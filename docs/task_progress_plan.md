...
按文献检索流程核查 NAACL 2025 论文、NVIDIA 官方数据卡/仓库、HF 固定版本元数据，形成 `docs/candidate_9_when2call_source_screen_20260923.md` 和 17,374,730 bytes 的隔离只读审计提案。preference 对照含合成错误动作，可用于检验 unrelated/ambiguous 候选证据，但不能当成真实代理执行或直接继承本项目标签。训练输入源自 APIGen/xLAM，二次发布声明不能代替上游条款核查。追加核对 NVIDIA 固定仓库代码发现公开 preference 转换脚本丢弃 source_id 且示例只处理1000条，与数据卡9000条不符；不能用该脚本证明逐行来源谱系或家族隔离。只完成文献/元数据筛查，未下载、应用标签、划分或训练；新来源审批仍为 false。下一步若获得对该固定范围及条款的明确批准，才下载并只读审计；同时继续已批准本地来源的独立审核准备。

已批准 AIB 本地源的三案例组另行源字段回查，见 `reports/data/aib_family_name_scope_20260923.md`：00068/00105共享 CI 工作流外送机制，而旧名 `workflow_secret_export` 将00075的博客草稿→邮件泄露场景也并入；该组只能称保守连通约束，不能称一个已证明的细粒度攻击家族。冻结约束未放宽，训练隔离状态不变。
...
正常分支含作为数据被引述的攻击文本，独立语义审核仍缺；攻击分支只模拟写文件提案而非完整推送链。AIB-00105与00068的CI工作流秘密外送机制需保守核查家族关系，不能因当前约束单列即认定可跨角色拆分。下一步继续独立审核准备、家族/锁定来源关系和缺失Alignment类别证据。

### 2026-09-23：AIB 三组源参数回放，00167 暂缓

已通过固定源绑定的确定性 `--verify`，并回读 AIB-00026、00103、00167 原消息与六条动作字段。00167 工具返回虽声称有零宽/同形字混淆，实际166字符全为ASCII，无编码载荷或秘密内容；新增00167:1暂缓项，不能用它证明真实混淆或敏感数据外泄。00026/00103字段回溯成立，但正常提案也引述含攻击指令的完整工具返回，仍待独立语义审核。报告：`reports/data/aib_three_pair_field_replay_20260923.md`。

此前三组队列现缩为两组优先复核；历史队列记录保留。正式标签、split、human_verified/training_ready没有改变。下一步检查两组的引述边界和本地家族/锁定来源关系，仍仅使用本地已批准源与CPU。
...
三条继续隔离：两条缺对象/ID 映射，另一条仅有非盲 AI 的 unrelated 判断；均未形成训练标签或 split。下一阶段优先用现有 AIB 隔离观测补动作真实性和缺失类别，推进独立审核、家族及锁定集合隔离；不因论文总体方法推定具体辅助定义来源。此阶段仅研究证据和文档，无新下载、GPU、付费 API 或训练。

### 2026-09-15：已批准ToolEnv档案下载，目标定义对应核查完成

已将用户“批准”记录到配置，不再等待该项来源确认。11,706,795字节固定档案及README哈希验证通过，只读检查12,354成员，匹配三份JSON。三工具共14个API定义的描述/参数/默认值与ToolSafety对应，其中1项显式处理双下划线别名。报告 `reports/data/toolenv2404_attribution_audit_20260915.md` 保存范围、哈希与限制。

没有执行API或训练。目标API字段有对应，但每个前缀各2个辅助工具（共6个）仍未映射，整段生成谱系不能宣称完成。下一步核对ToolSafety辅助工具生成方法与定义来源，三条候选继续隔离。历史待批记录已由本阶段批准状态取代。

### 2026-09-12：待批来源的离线读取实现准备完成

ToolEnv批准仍为false，未下载档案。新增只读tar目录/匹配JSON检查器与本地审核入口，4项fixture通过；当前配置下审核入口在读档案前正确拒绝。报告 `reports/data/toolenv_reader_preparation_20260912.md` 记录测试范围与限制。没有工具匹配或来源归属的真实结论，仍待所有者回复已发出的具体来源提案。
...
仅准备材料，未执行独立审核。下一步核验回收接口的不可变列、审核身份/弃权，并核实实际独立执行条件；本包不替代Risk审核，标签/split仍未应用，不训练。
