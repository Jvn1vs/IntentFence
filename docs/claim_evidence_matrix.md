...
| C3B-API | The service exposes version metadata and applies tool-specific failure policy. | TestClient fixture, API schema and policy tests; one local Uvicorn rules-only smoke. | Rules/mock tests and the 2026-09-02 local Uvicorn smoke cover application/model version fields, `/health`, external-communication blocking and read fail-open/external fail-closed. | `smoke-tested` | May be described as engineering behavior, not model safety. |
