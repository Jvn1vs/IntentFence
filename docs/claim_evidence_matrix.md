...
| C3B-API | The service exposes version metadata and applies tool-specific failure policy. | TestClient fixture, API schema and policy tests. | Rules/mock tests cover application/model version fields and read fail-open/external fail-closed. | `smoke-tested` | May be described as engineering behavior, not model safety. |
...
| RELEASE-SAFETY | Public source excludes raw data, credentials, checkpoints and ignored caches. | `scripts/audit_public_release.py`, Git tracked-file review and license checklist. | Automated audit is the C4 deliverable; final public approval is still pending. | `smoke-tested` | Do not publish until the audit and owner approval pass. |
