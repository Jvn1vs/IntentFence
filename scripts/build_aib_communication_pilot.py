"""Build the approved two-case communication pilot, or verify saved records."""
...
CONFIG = ROOT / "configs/aib_communication_pilot_20260911.yaml"
...
def main() -> None:
    ...
    cases = [CommunicationCase.model_validate(c) for c in json.loads(
    ...
    results = [bind_communication(rows[c.source.case_id], c) for c in cases]
    if results != [bind_communication(rows[c.source.case_id], c) for c in cases]:
    ...
    allowed = (ROOT / "data/interim/aib_communication_pilot_20260911").resolve()
    ...
    hashes = {str(p.relative_to(ROOT)).replace("\\", "/"): sha(p) for p in [
        ...
        ROOT / "src/intentfence/aib_communication_pilot.py",
        ROOT / "src/intentfence/communication_actions.py",
        ROOT / "src/intentfence/offline_actions.py",
    ]}
    manifest = {"hashes": hashes, "records_sha256": hashlib.sha256(records.encode()).hexdigest(),
                "cases": len(results),
                "proposals": sum(r["status"] == "proposal_quarantined" for r in results),
                "clarifications": sum(r["status"] == "missing_fields_quarantined" for r in results),
